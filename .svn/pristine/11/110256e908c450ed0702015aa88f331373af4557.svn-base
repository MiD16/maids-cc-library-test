...
<div class="container-fluid">
	<div class="row justify-content-center">
		<div class="col-12 ">
			...
			<div class="row mt-3">
				<div class="col-md-10">
					<a href="<c:url value='/interestRateSegment/create' />">
						<button type="button" class="btn mt-1 mb-0 btn-outline-primary"
							href="<c:url value='/interest/create' />">
							إضافة شريحة نسبة  فائدة <i class="fe fe-plus-circle" style=""></i>
						</button>
					</a>
				</div>
				<div class="col-md-2"
			...
			<div class="panel panel-default">
											...
											<table class="table datatables" id="dataTable-1">
												...
												<tbody>
													...
													<c:forEach varStatus="status" var="varuserList"
														items="${interestRateSegmentsList}">
														<tr>
															<td>${varuserList.id}</td>
															<td class="overflow-text-td">
															
															 ${varuserList.interestRate.interest.descriptionA}/${varuserList.interestRate.currency.nameA}/ %${varuserList.interestRate.rateValue}
															...
															<td>
															من
															${varuserList.currencySegments.currency.iso}
															<span  class="input-money">${varuserList.currencySegments.fromSeg} </span>
															إلى
															${varuserList.currencySegments.currency.iso}
															<span  class="input-money">${varuserList.currencySegments.toSeg}</span>
																
															</td>
															<td class="overflow-text-td">${varuserList.rate} % </td>
															<td><i id="delete_${status.index}"
																data-id="${status.index}"
																onclick="window.location.href='${pageContext.request.contextPath}/interestRateSegment/delete?id=${varuserList.id} '"
																class=" fe fe-delete icon-delete"></i> &nbsp&nbsp&nbsp <i
																id="edit_${status.index}" data-id="${status.index}"
																onclick="window.location.href='${pageContext.request.contextPath}/interestRateSegment/update?id=${varuserList.id} '"
																class=" fe fe-edit icon-edit"> </i> &nbsp&nbsp<i
																id="edit_${status.index}" data-id="${status.index}"
																onclick="window.location.href='${pageContext.request.contextPath}/interestRateSegment/details?id=${varuserList.id} '"
				...
				<jsp:include page="/WEB-INF/jsp/layout/pagination.jsp">
					<jsp:param name="controller_url" value="/interestRateSegment/index" />
				</jsp:include>
			</div>
